...
primary_ip=$(echo "${oracle_nodes_json}" | jq -r '.[] | select(.name | endswith("-001")) | .ip')
if [[ -z "$primary_ip" ]]; then
  echo "ERROR: Could not find a primary node ending with '-001'."
  exit 1
fi
...
echo "${oracle_nodes_json}" | jq -c '.[]' | while read -r node; do
  node_name=$(echo "$node" | jq -r '.name')
  node_ip=$(echo "$node" | jq -r '.ip')
  node_zone=$(echo "$node" | jq -r '.zone')

  # Triggering SSH key creation via OS Login by running a one-time gcloud compute ssh command.
  # This ensures that a persistent SSH key pair is created and associated with your Google Account.
  # The private auto-generated ssh key (~/.ssh/google_compute_engine) will be used by Ansible 
  # to connect to the VM and run playbooks remotely.
  ...
  if [[ "$node_name" == *"-001" ]]; then
    echo "Configuring PRIMARY node: $node_name"
    bash install-oracle.sh --cluster-type NONE --instance-ip-addr "$node_ip" "$common_flags"
  else
    echo "Configuring STANDBY node: $node_name"
    bash install-oracle.sh --cluster-type DG --primary-ip-addr "$primary_ip" --instance-ip-addr "$node_ip" "$common_flags"
  fi
done
